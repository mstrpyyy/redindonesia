...
import { MAX_PODCAST_BANNER_LABEL, MAX_PODCAST_BANNER_VIDEO_LABEL } from "./limits";
import { savePodcastPage, uploadPodcastPageBanner, uploadPodcastPageBannerVideo } from "./actions";
import type { IPodcastPage, PodcastPageSlug } from "@/lib/podcast-page";

export function PodcastPageForm({
  slug,
  initialData,
}: {
  slug: PodcastPageSlug;
  initialData: IPodcastPage;
}) {
  ...
  const handleSave = () => {
    ...
    startTransition(async () => {
      ...
      const result = await savePodcastPage(slug, formData);
      setMessage(
        result.success
  ...
  return (
    <div className="flex flex-col gap-4">
      <PageBannerFields
        ...
        uploadImageAction={uploadPodcastPageBanner}
        uploadVideoAction={uploadPodcastPageBannerVideo}
        imageSizeLabel={MAX_PODCAST_BANNER_LABEL}
        videoSizeLabel={MAX_PODCAST_BANNER_VIDEO_LABEL}
        disabled={isPending}
      />
